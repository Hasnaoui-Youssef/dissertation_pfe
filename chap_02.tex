...
Identifying a defect requires a specification to compare against. A program required to toggle an LED once per second provides a simple example. An LED that remains static, or that toggles at a different rate, is a case of a program violating its specification.

A specification states an end result, and that result implies intermediate behavior. Toggling an LED on a microcontroller requires initializing the device, configuring its power supply and clock tree, and writing to the registers that drive the output pin. A fault at any of these stages produces the same observable symptom, namely an LED that does not blink at the required rate. Writing to an incorrect register and omitting the clock configuration are distinct defects with identical symptoms.
...
A \textit{hardware breakpoint} instead configures dedicated hardware components on the device such as comparators that watch the address of instructions fetched and raises a debug event on a match without midoifying the program. Figure \ref{fig:breakpoints} contrasts the two mechanisms.
...
An exception requires a handler, which is a subroutine that is executed whenever the exception occurs. And the address of the entirety of the handlers are stored contiguously in a table termed the Vector Table, which is then indexed by the number of the exception to retrieve the appropriate handler.
...
\begin{itemize}
    ...
    \item SWJ Debug Port which comibenes the two above\cite{adiv5}.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption[Structure of a formatter frame]{Structure of a formatter frame, taken from \cite{coresight_arch}}
