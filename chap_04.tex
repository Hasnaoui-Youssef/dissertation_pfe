...
This section states the solution as a whole before the later sections detail its parts. It presents the global architecture, the capabilities the engine provides, and the design rules that every component observes.
...
The stages are separated because each depends on a different thing. Generation and capture depend on the device, extraction on the debug connection, decoding on the trace protocol, and analysis on the program image. A stage that combined two of these could run only when both were present.
...
The separation has four consequences. The work that needs the device is held apart from the work that does not, so the two do not interleave. The last two stages run on a stored capture, so a trace is analyzed offline with no device present. Each stage is tested on its own against fixed data. A stage is reused where its inputs allow, since decoding and analysis depend on the capture and the image alone.
