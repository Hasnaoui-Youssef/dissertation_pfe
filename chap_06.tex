...
Table \ref{tab:correctness} summarizes the four checks and what each established.

\begin{table}[htbp]
    \centering
    \caption{Correctness checks and their outcomes}
    \label{tab:correctness}
    \begin{tabularx}{\linewidth}{| @{}>{\hsize=0.7\hsize\bfseries}Y | >{\hsize=1.45\hsize}Y | >{\hsize=0.85\hsize}Y@{} |}
        \hline
        Check & \textbf{What was compared} & \textbf{Outcome} \\
        \hline
        Stored capture & Reconstructed instruction count against the frozen baseline, and every address against the static analysis & Counts match, every address resolves \\
        \hline
        Device & Trace-unit registers against the values recorded, and callbacks delivered on a halt & Registers match, two callbacks per halt \\
        \hline
        Commercial debuggers & Decoded listing against three vendor debuggers, instruction by instruction, each driving its own probe & Sequences agree, divergences confined to presentation \\
        \hline
        Cross-vendor capture & Byte stream against the block drained from the on-chip sink & Identical once the synchronization framing is discounted \\
        \hline
    \end{tabularx}
\end{table}
...
Each requirement is traced to the component that provides it and the section that realizes it. Table \ref{tab:rtm} states that mapping, and Table \ref{tab:verification} records how each requirement was checked.
...
The work meets its objectives within limits, and those limits point to directions for extending it. The limitations fall into two groups. Table \ref{tab:trace_limits_tab} states the limitations of the trace pipeline, the stage that turns a capture into a reconstructed program. The limitations of the engine that carries the pipeline follow the table, together with the directions that extend the work.
